...
extern "C" {
	...
	unsigned char read_public_key_from_string(const char *input, char *key_type, unsigned char *public_key);
	...
	unsigned char decrypt_aes_key_with_ml_kem(const unsigned char *private_kem_key, uint64_t kem_key_size, const unsigned char *ciphertext, unsigned int ciphertext_length, unsigned char *decrypted_aes_key);

	unsigned char get_signature_size(const unsigned char *private_key, uint64_t key_size, const unsigned char *data, uint64_t data_size, uint64_t *signature_size);

	unsigned char sign_with_ml_dsa(const unsigned char *private_key, uint64_t key_size, const unsigned char *data, uint64_t data_size, unsigned char *signature, uint64_t *signature_size);

	int verify_with_ml_dsa(const unsigned char *public_key, int key_size, const unsigned char *data, uint64_t data_size, const unsigned char *signature, unsigned int signature_size);
}
